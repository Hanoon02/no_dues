...
function clickFilter() {
  ...
  var currentList = [];
  studentList.map(function (student) {currentList.push(student);});
...
function applyStatus(){
  ...
  request.open("GET", `${CURRENT_URL}/staff/getStudents/${adminName}/${curr_status}`, false);
  request.send(null);
...
function approved(e) {  
  ...
  request.open(
    "GET",
    `${CURRENT_URL}/staff/approveDues/${JSON.stringify(obj)}`,
    false
  );
  request.send(null);
  applyStatus();
...
sheet.addEventListener("click", () => {
  ...
  window.open(`${CURRENT_URL}/staff/showSheet`, "_blank");
});
